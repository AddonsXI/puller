-- Palette and style, following luashitaview's by way of kupoquest's copy.
-- Enums are read by name out of _G and nil guarded, so the headless suite can load this
-- with no imgui. A renamed enum then does nothing rather than throwing.
...
-- The only field in the window you type into, sunk almost to black.
M.inputBg = { 0.03, 0.03, 0.04, 1.00 };

-- One color per token, worn by its name in the reference line and by the value it
-- produces in the preview. The pairing is what says which variable became which word.
M.token = {
    name       = { 0.98, 0.80, 0.35, 1.00 },
    level      = { 0.45, 0.78, 0.98, 1.00 },
    difficulty = { 0.96, 0.55, 0.40, 1.00 },
    attributes = { 0.76, 0.62, 0.98, 1.00 },
    call       = { 0.44, 0.86, 0.58, 1.00 },
};
